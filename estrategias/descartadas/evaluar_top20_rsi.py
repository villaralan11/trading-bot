...
# ============================================================
# ESTRATEGIA RSI MEAN-REVERSION (COPIA EXACTA DE estrategia_rsi_1h.py)
# ============================================================
...
def run_backtest(par, csv_path):
    ...
    # Also get raw trade data for aggregation
    trades_data = []
    if hasattr(strat, 'trades_raw'):
        trades_data = strat.trades_raw
...
# ============================================================
# MAIN: EJECUTAR EN IN-SAMPLE DE LOS 20 PARES
# ============================================================
if __name__ == '__main__':
    pares = [
        ('BTC/USDT', '/home/alancito/trading-bot/datos/historicos/BTC_USDT_1h_4y_in_sample.csv'),
        ('ETH/USDT', '/home/alancito/trading-bot/datos/historicos/ETH_USDT_1h_4y_in_sample.csv'),
        ('RE/USDT', '/home/alancito/trading-bot/datos/historicos/RE_USDT_1h_4y_in_sample.csv'),
        ('SOL/USDT', '/home/alancito/trading-bot/datos/historicos/SOL_USDT_1h_4y_in_sample.csv'),
        ('ZEC/USDT', '/home/alancito/trading-bot/datos/historicos/ZEC_USDT_1h_4y_in_sample.csv'),
        ('TREE/USDT', '/home/alancito/trading-bot/datos/historicos/TREE_USDT_1h_4y_in_sample.csv'),
        ('BNB/USDT', '/home/alancito/trading-bot/datos/historicos/BNB_USDT_1h_4y_in_sample.csv'),
        ('SNDKB/USDT', '/home/alancito/trading-bot/datos/historicos/SNDKB_USDT_1h_4y_in_sample.csv'),
        ('XRP/USDT', '/home/alancito/trading-bot/datos/historicos/XRP_USDT_1h_4y_in_sample.csv'),
        ('DEXE/USDT', '/home/alancito/trading-bot/datos/historicos/DEXE_USDT_1h_4y_in_sample.csv'),
        ('ADA/USDT', '/home/alancito/trading-bot/datos/historicos/ADA_USDT_1h_4y_in_sample.csv'),
        ('DOGE/USDT', '/home/alancito/trading-bot/datos/historicos/DOGE_USDT_1h_4y_in_sample.csv'),
        ('NEAR/USDT', '/home/alancito/trading-bot/datos/historicos/NEAR_USDT_1h_4y_in_sample.csv'),
        ('BANK/USDT', '/home/alancito/trading-bot/datos/historicos/BANK_USDT_1h_4y_in_sample.csv'),
        ('ONDO/USDT', '/home/alancito/trading-bot/datos/historicos/ONDO_USDT_1h_4y_in_sample.csv'),
        ('U/USDT', '/home/alancito/trading-bot/datos/historicos/U_USDT_1h_4y_in_sample.csv'),
        ('ADA/USDT', '/home/alancito/trading-bot/datos/historicos/ADA_USDT_1h_4y_in_sample.csv'),
        ('DOGE/USDT', '/home/alancito/trading-bot/datos/historicos/DOGE_USDT_1h_4y_in_sample.csv'),
        ('NEAR/USDT', '/home/alancito/trading-bot/datos/historicos/NEAR_USDT_1h_4y_in_sample.csv'),
        ('BANK/USDT', '/home/alancito/trading-bot/datos/historicos/BANK_USDT_1h_4y_in_sample.csv'),
    ...
    for par, path in pares:
        if par not in seen:
            seen.add(par)
            unique_pares.append((par, path))
    
    print(f"\n{'='*80}")
    print(f"EVALUACIÓN RSI MEAN-REVERSION EN 20 PARES (IN-SAMPLE 1h, 4 AÑOS)")
    print(f"Parámetros: RSI 14, compra <30, vende >50, SL 4%, filtro SMA200, 20% sizing")
    print(f"{'='*80}")
    
    resultados = []
    
    for par, path in unique_pares:
        try:
            print(f"\n--- Procesando {par} ---")
    ...
    # Tabla comparativa
    print(f"\n{'='*100}")
    print(f"TABLA COMPARATIVA RSI MEAN-REVERSION (IN-SAMPLE 1h, 4 AÑOS, 20 PARES)")
    print(f"{'='*100}")
    ...
    if resultados:
        ...
        print(f"\nPares con ≥30 trades: {pares_30}/{len(resultados)}")
    
    # ============================================================
    # RESUMEN AGREGADO: Sumar todos los trades como una sola muestra
    # ============================================================
    print(f"\n{'='*100}")
    print(f"RESUMEN AGREGADO (TODOS LOS TRADES DE LOS 20 PARES COMBINADOS)")
    print(f"{'='*100}")
    
    total_trades_all = sum(r['trades'] for r in resultados)
    total_won_all = sum(r['ganadores'] for r in resultados)
    total_lost_all = sum(r['perdedores'] for r in resultados)
    agg_win_rate = (total_won_all / total_trades_all * 100) if total_trades_all > 0 else 0
    avg_retorno = sum(r['retorno_pct'] for r in resultados) / len(resultados)
    avg_sharpe = sum(r['sharpe'] for r in resultados) / len(resultados)
    avg_max_dd = sum(r['max_dd_pct'] for r in resultados) / len(resultados)
    
    print(f"Total trades (20 pares combinados):     {total_trades_all}")
    print(f"  Ganadores: {total_won_all}")
    print(f"  Perdedores: {total_lost_all}")
    print(f"  Win Rate agregado: {agg_win_rate:.2f}%")
    print(f"  Retorno promedio ponderado: {avg_retorno:.2f}%")
    print(f"  Sharpe promedio: {avg_sharpe:.2f}")
    print(f"  Max DD promedio: {avg_max_dd:.2f}%")
    print(f"\n  → CONCLUSIÓN: {'HAY VENTAJA REAL' if agg_win_rate > 50 and avg_retorno > 0 else 'NO HAY VENTAJA ESTADÍSTICA CLARA'}")
    print(f"{'='*100}")
