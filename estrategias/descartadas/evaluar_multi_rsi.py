...
class EstrategiaRSI(bt.Strategy):
    params = (
        ('periodo_rsi', 14),
        ('rsi_sobreventa', 30),
        ('rsi_sobrecompra', 50),  # salida cuando RSI sube de 50 (no 70)
    ...
    def __init__(self):
        self.rsi = bt.indicators.RSI(self.data.close, period=self.params.periodo_rsi)
    ...
    def notify_order(self, order):
        ...
        if order.status == order.Completed:
            if order.isbuy():
                ...
                self.colocar_sl(order.executed.price, order.executed.size)
            elif order.issell():
    ...
    def colocar_sl(self, precio_entrada, size):
    ...
    def cancelar_sl(self):
        if self.sl_order and self.sl_order.status in [self.sl_order.Submitted, self.sl_order.Accepted]:
            self.cancel(self.sl_order)
            self.sl_order = None
    ...
    def next(self):
        ...
        if self.rsi[0] < self.params.rsi_sobreventa and tendencia_alcista and not self.position:
            self.log(f'SEÑAL COMPRA: RSI={self.rsi[0]:.1f} < {self.params.rsi_sobreventa} (sobreventa), Precio={self.data.close[0]:.2f} > SMA200={self.sma_tendencia[0]:.2f} (TENDENCIA ALCISTA)')
        ...
        elif self.rsi[0] > self.params.rsi_sobrecompra and self.position:
            self.log(f'SEÑAL VENTA (RSI > {self.params.rsi_sobrecompra}): RSI={self.rsi[0]:.1f}, Precio={self.data.close[0]:.2f}')
            self.cancelar_sl()
            self.orden_principal = self.sell(size=self.position.size)
...
# ============================================================
# MAIN: EJECUTAR EN IN-SAMPLE DE LOS 5 PARES
# ============================================================
if __name__ == '__main__':
    pares = [
        ('BTC/USDT', '/home/alancito/trading-bot/datos/historicos/btc_usdt_4h_in_sample.csv'),
        ('ETH/USDT', '/home/alancito/trading-bot/datos/historicos/ETH_USDT_4h_in_sample.csv'),
        ('BNB/USDT', '/home/alancito/trading-bot/datos/historicos/BNB_USDT_4h_in_sample.csv'),
        ('SOL/USDT', '/home/alancito/trading-bot/datos/historicos/SOL_USDT_4h_in_sample.csv'),
        ('ADA/USDT', '/home/alancito/trading-bot/datos/historicos/ADA_USDT_4h_in_sample.csv'),
    ]
    
    print(f"\n{'='*80}")
    print(f"EVALUACIÓN RSI MEAN-REVERSION EN 5 PARES (IN-SAMPLE 4H)")
    print(f"Parámetros: RSI(14), compra <30, venta >50, SL 4%, sizing 20%")
    print(f"{'='*80}")
    ...
    for par, path in pares:
        try:
            ...
            print(f"  Retorno: {res['retorno_pct']:.2f}%, Trades: {res['trades']}, Win rate: {res['win_rate']:.1f}%, Sharpe: {res['sharpe']:.2f}, Max DD: {res['max_dd_pct']:.2f}%")
    ...
    # Tabla comparativa
    print(f"\n{'='*100}")
    print(f"TABLA COMPARATIVA RSI MEAN-REVERSION (IN-SAMPLE 4H)")
    print(f"{'='*100}")
    ...
    if resultados:
        ...
        print(f"{'PROMEDIO':<12} {'':>7} {'':>8} {'':>9} {avg_winrate:>9.1f}% {avg_retorno:>10.2f}% {avg_sharpe:>8.2f} {avg_dd:>9.2f}%")
    
    print(f"\n{'='*100}")
    print(f"CONCLUSIÓN: ¿La estrategia RSI mean-reversion generaliza mejor que SMA crossover?")
    print(f"  - SMA crossover (20/50/200): Promedio -3.15% retorno, 37% win rate, Sharpe -0.28")
    print(f"  - RSI mean-reversion: Promedio {avg_retorno:.2f}% retorno, {avg_winrate:.1f}% win rate, Sharpe {avg_sharpe:.2f}")
    print(f"{'='*100}")
