...
class EstrategiaTendenciaSMA(bt.Strategy):
    ...
    def stop(self):
        ...
        print(f"\n{'='*60}")
        print(f"RESULTADOS BACKTEST - TENDENCIA SMA 4H + CRUCE + SL/TP (VALIDACIÓN OUT-SAMPLE)")
        print(f"{'='*60}")
...
df = pd.read_csv('/home/alancito/trading-bot/datos/historicos/btc_usdt_4h_out_sample.csv')
...
try:
    fig = cerebro.plot(style='candlestick', barup='green', bardown='red', 
                       volume=False, 
                       savefig=dict(fname='resultado_v3_validacion.png', dpi=150, bbox_inches='tight'))[0][0]
    print("Gráfica guardada como: resultado_v3_validacion.png")
