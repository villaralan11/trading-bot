...
class EstrategiaTendenciaSMA(bt.Strategy):
    ...
    def stop(self):
        ...
        print(f"\n{'='*60}")
        print(f"RESULTADOS BACKTEST - TENDENCIA SMA + CRUCE + SL/TP")
        print(f"{'='*60}")
...
df = pd.read_csv('/home/alancito/trading-bot/datos/historicos/btc_usdt_1h_in_sample.csv')
...
try:
    fig = cerebro.plot(style='candlestick', barup='green', bardown='red', 
                       volume=False, 
                       savefig=dict(fname='resultado_v2.png', dpi=150, bbox_inches='tight'))[0][0]
    print("Gráfica guardada como: resultado_v2.png")
