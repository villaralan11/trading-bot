...
print(f"Encontrados {len(files_1h_4y)} archivos 1h_4y")
...
for filename in files_1h_4y:
    ...
    # Check if enough data for meaningful split
    if len(df) < 1000:
        print(f"  ⚠️ POCO DATA ({len(df)} velas) - saltando split")
        continue
    ...
    print(f"  IN: {len(df_in)} filas ({df_in.index[0]} a {df_in.index[-1]})")
    print(f"  OUT: {len(df_out)} filas ({df_out.index[0]} a {df_out.index[-1]})")
